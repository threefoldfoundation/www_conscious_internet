...
<page-query>
query ($private: Int){
  entries: allPerson (sortBy: "rank", order: ASC, filter: { private: { ne: $private }, category: { contains: ["aci"]}}){
    totalCount
    edges {
      node {
        path
...
export default {
  ...
  metaInfo() {
    return {
      title: "",
      titleTemplate: " ACI | People",
      meta: [
        {
          key: "description",
          name: "description",
          content:
            "Meet the wide range of passionate and dedicated individuals who believe in the need for a conscious Internet.",
        },
        {
          key: "og:title",
          property: "og:title",
